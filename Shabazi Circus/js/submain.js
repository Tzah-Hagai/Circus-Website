    ...
    var formInputs = $('.wrap-input100')
        ...
        myform.submit(function (event) {
            ...
            if(!formInputs.hasClass('alert-validate')) {
                myform.find("#validationAlert").text("שולח הודעה...");
                emailjs.sendForm(service_id, template_id, "myform")
                    .then(function () {
                        myform.find("#validationAlert").text("הודעתך נשלחה!");
                        myform.find("#validationAlert").css('color', 'green');
                    }, function (err) {
                        alert("תקלה בשליחת ההודעה!\r\n Response:\n " + JSON.stringify(err));
                        myform.find("button").text("שלח הודעה");
                    });
                return false;
            } else {
                myform.find("#validationAlert").text("אנא מלאו את כל השדות");
                return false;
            }
        });
